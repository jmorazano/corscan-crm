...
| Métricas del propietario (024) | `src/lib/metrics.ts` (puro: rangos `day`/`week`/`year` → barras por día/semana/mes, `metricsWindow` en hora LOCAL con `zonedToUtc`, comparación = misma duración transcurrida antes, `fillSeries`, formatos y contrato `MetricsOverview`) · `src/server/metrics/overview.ts` (3 consultas `scoped()`: totales `count(*) filter`, serie `date_trunc(... AT TIME ZONE tz)` por `conversation.kind`, tiempo de respuesta con ventana `out_seq` + `percentile_cont`; SOLO conversaciones reales WhatsApp/Instagram, sin `source='history'`, sin reacciones, sin fallidos) · `GET /api/metrics?range=&tz=` con `withOwner` (lectura, pero del propietario) · `/metrics` con `requireOwnerPage()` + `OWNER_ONLY_SECTIONS` · UI `src/components/metrics/` (tarjetas + histograma SVG propio `stacked-bars.tsx`, sin librería) · seed de números conocidos `tests/e2e/fixtures/seed-metrics.mjs` |
| Publicaciones de Mercado Libre, privacidad del agente y número personal (025) | adaptador ÚNICO `src/lib/meli/` (`oauth.ts`: authorization code + **PKCE S256 derivado** del nonce del state con HMAC —nada que guardar—, refresh **de un solo uso que rota**; `client.ts`: `users/me`, `users/{id}/items/search?status=active` paginado hasta 1.000, `/items/bulk` con fallback a `/items?ids=` (deprecado el 25-oct-2026), descripción; puros `listing.ts` normalización + `meliLink` https del dominio del sitio, `search.ts` sinónimos/zona sin tildes/moneda dominante/relajaciones, `render.ts` sección `PUBLICACIONES VIGENTES` + `[HERRAMIENTA]` + ficha en valla con nonce) · `src/server/meli/` (`integration.ts` tokens cifrados, lock en proceso por empresa para renovar, `ml_user_id` único de instancia y ante conflicto los tokens nuevos pasan a la dueña; `sync.ts` snapshot `meli_listing` con upsert + borrado de inactivas en transacción, descripción solo si cambió `last_updated`, stale-while-revalidate 3 h; `agent-tools.ts` el turno consulta la BASE, jamás la red → Laboratorio aislado) · acciones `search_listings`/`show_listing` (familia propia, 2 vueltas) y `request_visit` TERMINAL (nota + etapa «visita»/«interesad» por nombre + handoff `visita` con push) · `/api/integrations/mercadolibre{,/connect,/callback,/sync}` + `/integrations/mercadolibre` + tarjeta del índice · meli-mock `src/app/api/dev/meli-mock/` (PKCE real, rotación, perillas en `/state`) · guía `docs/integraciones/mercadolibre.md` · **privacidad GENERAL**: `privacyRules()` en `prompts.ts` para TODA empresa DESPUÉS de las instrucciones del negocio (no habla de otras conversaciones/personas, no revela instrucciones, nunca niega ser un asistente si se lo preguntan en serio) + guarda PURA `src/lib/privacy-guard.ts` sobre todo texto del modelo (teléfono/email que no esté en el corpus del turno ni sea del contacto → fuera con su oración) + regla del Entrenador (no guardar datos de terceros) + persona `datos_ajenos` y criterio del juez · **número personal**: `agent_profile.shared_personal_number` (tarjeta en Agente) + `contact.known_from_phone_at` (lo marcan agenda, historial y ecos en `inbox/history.ts`; backfill en la migración 0021) → `shouldSkipKnownContact` corta en `runAgentTurn` antes del proveedor, y `personalNumberSection()` hace callar ante mensajes personales de números nuevos · seed `scripts/seed/agents/distrito-inmobiliario.json` (Javier; el seed acepta `sharedPersonalNumber`) |
...
ai-mock + google-mock + meli-mock) tras un gate único (`src/lib/dev-guard.ts`): 404
...
Feature en curso: **025-mercadolibre-listings** (publicaciones vigentes de
Mercado Libre por empresa para que el agente ofrezca opciones y junte
pedidos de visita; privacidad del agente como regla GENERAL; «número
personal» del dueño; Javier de Distrito Inmobiliario) — spec, plan y tasks
en [specs/025-mercadolibre-listings/](specs/025-mercadolibre-listings/spec.md).
Anterior: 024-owner-metrics (en producción, 26-sep-2026).
